...
def calculate_accessibility_with_times(walking_distance_file, od_data_file, accessibility_file, walking_speed_kmh=4.8):
    ...
    access times for origin-destination pairs (ACCESS ONLY - NO EGRESS).
    ...
    walking_speed_kmh: Walking speed in km/hour (default 4.8 km/h = 80 m/min)
    ...
    Total Journey Time (ACCESS ONLY):
    - Access walk ONLY (from origin zone to departure stop)
    - No egress calculation
    """
    ...
    print(f"Starting processing (ACCESS ONLY - no egress)...")
    ...
    for _, acc_row in accessibility_df.iterrows():
        ...
        destination_zone = acc_row['Destino_ID']
        ...
        if not od_match.empty:
            ...
            if uses_public_transport:
                ...
                access_time_minutes = travel_time
...
def analyze_accessibility_coverage(results_df, destination_type='all'):
    ...
    print("\n=== ACCESSIBILITY ANALYSIS SUMMARY (ACCESS ONLY) ===")
    ...
    if not valid_journeys.empty:
        print(f"\nAccess Time Statistics (valid journeys: {len(valid_journeys)}):")
        print(f"Average access time: {valid_journeys['Access_Time_Minutes'].mean():.2f} minutes")
        print(f"Median access time: {valid_journeys['Access_Time_Minutes'].median():.2f} minutes")
        print(f"Min access time: {valid_journeys['Access_Time_Minutes'].min():.2f} minutes")
        print(f"Max access time: {valid_journeys['Access_Time_Minutes'].max():.2f} minutes")
...
if __name__ == "__main__":
    ...
    if not results.empty:
        ...
        sample_cols = ['Origin', 'Destination', 'Journey_Type', 'Access_Time_Minutes']
