...
class AccessibilityAnalyzer:
    ...
    def __init__(self, df: pd.DataFrame, time_metric: str = 'JRT'):
        self.df = df.copy()
        self.time_metric = time_metric  # 'JRT' or 'PJT'

        # Select source column based on chosen metric
...
class StreamlitApp:
    ...
    def _render_metric_selector(self, df: pd.DataFrame) -> str:
        """Render the travel time metric selector in the sidebar and return chosen metric key"""
        st.sidebar.markdown("## ⚙️ Analysis Settings")
        has_pjt = 'Tiempo_Viaje_Percibido' in df.columns

        if has_pjt:
            metric_choice = st.sidebar.radio(
                "**Travel time metric**",
                options=["JRT – Journey Time", "PJT – Perceived Journey Time"],
                index=0,
                help=(
                    "**JRT** uses the raw journey time (Tiempo_Trayecto).\n\n"
                    "**PJT** uses the perceived journey time (Tiempo_Viaje_Percibido), "
                    "which applies weighting penalties to waiting, walking, and transfer time."
                )
            )
            return "PJT" if metric_choice.startswith("PJT") else "JRT"
        else:
            st.sidebar.info("ℹ️ PJT column (Tiempo_Viaje_Percibido) not found in data. Using JRT.")
            return "JRT"

    def process_uploaded_file(self, uploaded_file, time_metric: str = 'JRT') -> Tuple[pd.DataFrame, pd.DataFrame, AccessibilityAnalyzer]:
        ...
        try:
            ...
            time_col = 'Tiempo_Viaje_Percibido' if (time_metric == 'PJT' and 'Tiempo_Viaje_Percibido' in df.columns) else 'Tiempo_Trayecto'
            filtered_df = df[df[time_col] <= 999]
            ...
            # Create analyzer with selected metric
            analyzer = AccessibilityAnalyzer(df, time_metric=time_metric)
    ...
    def render_key_metrics(self, analyzer: AccessibilityAnalyzer):
        ...
        with col3:
            st.metric(
                label=f"Avg Time per Zone ({analyzer.active_metric_label.split(' ')[0]})",
                value=f"{travel_metrics['avg_per_zone']:.1f} min"
            )

        with col4:
            st.metric(
                label=f"Avg Time Pop-Weighted ({analyzer.active_metric_label.split(' ')[0]})",
    ...
    def render_travel_time_distributions(self, analyzer: AccessibilityAnalyzer):
        ...
        st.markdown(f"## ⏱️ Travel Time Distribution Analysis — {analyzer.active_metric_label}")
    ...
    def _create_travel_time_distribution(self, analyzer: AccessibilityAnalyzer) -> go.Figure:
        ...
        return self.styler.apply_standard_styling(
            fig, f"Travel Time Distribution (Population-Weighted) — {analyzer.active_metric_label}",
    ...
    def _create_travel_time_by_zones_chart(self, analyzer: AccessibilityAnalyzer) -> go.Figure:
        ...
        return self.styler.apply_standard_styling(
            fig, f"Travel Time Distribution by Zones — {analyzer.active_metric_label}",
    ...
    def render_transport_comparison(self, analyzer: AccessibilityAnalyzer):
        ...
        col1.metric("Avg Public Transport", f"{avg_public:.1f} min",
                    help=f"Based on {analyzer.active_metric_label}")
        ...
        self._render_transport_comparison_chart(muni_comp, 'Municipality', 10, analyzer)
        ...
        self._render_transport_comparison_chart(comarca_comp, 'Comarca', 10, analyzer)
    ...
    def _render_transport_comparison_chart(self, analysis: pd.DataFrame,
                                           ...
                                           analyzer: 'AccessibilityAnalyzer' = None):
        """Render transport comparison chart"""
        chart_data = analysis.head(top_n) if top_n < len(analysis) else analysis
        metric_label = analyzer.active_metric_label.split(' ')[0] if analyzer else 'JRT'

        fig = go.Figure()

        fig.add_trace(go.Bar(
            name=f'Public Transport ({metric_label})',
            y=chart_data['Geographic Unit'],
    ...
    def run(self):
        ...
        if uploaded_file is not None:
            # Load file once to inspect columns, then show metric selector
            try:
            ...
            time_metric = self._render_metric_selector(df_preview)

            # Active metric badge
            if time_metric == 'PJT':
                st.info("🧠 **Active metric: Perceived Journey Time (PJT)** — analysis uses `Tiempo_Viaje_Percibido`")
            else:
                st.info("⏱️ **Active metric: Journey Time (JRT)** — analysis uses `Tiempo_Trayecto`")

            # Process data
            df, df_all, analyzer = self.process_uploaded_file(uploaded_file, time_metric=time_metric)
