...
def clean_accessibility_data(input_file, output_file=None):
    ...
    1. Use Access_Time_Minutes and Access_Distance_Meters if not empty/NaN
       Otherwise use Mapeada_ACT and Mapeada_ACD

    2. Use Egress_Time_Minutes and Egress_Distance_Meters if not empty/NaN
       Otherwise use Mapeada_EGT and Mapeada_EGD

    3. Recalculate journey time when using calculated access/egress times:
       New_Journey_Time = Mapeada_JRT - Mapeada_ACT - Mapeada_EGT + Final_Access_Time + Final_Egress_Time
    ...
    for idx, row in df_clean.iterrows():
        ...
        if (is_valid_value(row.get('Access_Time_Minutes')) and
            ...
            # Use mapped values
            df_clean.loc[idx, 'Tiempo_Acceso_Final'] = row.get('Mapeada_ACT', np.nan)
            df_clean.loc[idx, 'Distancia_Acceso_Final'] = row.get('Mapeada_ACD', np.nan) * 1000  # Convert km to meters
    ...
    for idx, row in df_clean.iterrows():
        ...
        if (is_valid_value(row.get('Egress_Time_Minutes')) and
            ...
            # Use mapped values
            df_clean.loc[idx, 'Tiempo_Salida_Final'] = row.get('Mapeada_EGT', np.nan)
            df_clean.loc[idx, 'Distancia_Salida_Final'] = row.get('Mapeada_EGD', np.nan) * 1000  # Convert km to meters
    ...
    for idx, row in df_clean.iterrows():
        ...
        if used_calculated_access or used_calculated_egress:
            ...
            ride_time = row.get('Mapeada_RIT', 0)
            ...
            df_clean.loc[idx, 'Tiempo_Viaje_Ajustado'] = row.get('Mapeada_JRT', np.nan)
            original_count += 1
    ...
    cleaned_df = pd.DataFrame({
        ...
        'Nombre_Zona_Origen': df_clean.get('Origin_ZoneName', ''),
        ...
        'Tiempo_Trayecto_Minutos': df_clean.get('Mapeada_RIT', np.nan),  # RIT: Ride Time only (vehicle in motion)
        'Numero_Transbordos': df_clean.get('Mapeada_NTR', np.nan),
        'Distancia_Viaje_Total_Km': df_clean.get('Mapeada_JRD', np.nan),
        'Distancia_Trayecto_Km': df_clean.get('Mapeada_RID', np.nan),
        'Tiempo_Espera_Transbordo_Minutos': df_clean.get('Mapeada_TWT', np.nan),
        'Frecuencia_Servicio': df_clean.get('Mapeada_SFQ', np.nan),

        # Original mapped values for reference
        'Tiempo_Acceso_Original': df_clean.get('Mapeada_ACT', np.nan),
        'Tiempo_Salida_Original': df_clean.get('Mapeada_EGT', np.nan),
        'Distancia_Acceso_Original_Km': df_clean.get('Mapeada_ACD', np.nan),
        'Distancia_Salida_Original_Km': df_clean.get('Mapeada_EGD', np.nan),
        'Tiempo_Viaje_Original_Minutos': df_clean.get('Mapeada_JRT', np.nan),
    })
...
if __name__ == "__main__":
    ...
    input_file = r"C:\Users\avoltan\PyCharmMiscProject\accessibility\accessibility_results_with_times.csv"
    output_file = "datos_accesibilidad_limpios.csv"
