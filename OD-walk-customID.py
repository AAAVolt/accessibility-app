...
def calculate_accessibility_with_times(walking_distance_file, od_data_file, accessibility_file, walking_speed_kmh=4.8):
    ...
    accessibility_file: CSV with accessibility analysis data (with Destino_ID and Mejor_* columns)
    ...
    - Walking Only: Direct walking time (from OD data Time_Minutes column)
    """
    ...
    # Check what destination values we have in accessibility file (using Destino_ID instead of Zona_Mapeada)
    print(f"Checking Destino_ID values...")
    destino_id_values = accessibility_df['Destino_ID'].dropna()
    unique_destinations = sorted(destino_id_values.unique())
    print(f"Unique Destino_ID values: {unique_destinations[:20]}...")  # Show first 20
    ...
    acc_destinations = set(int(x) for x in destino_id_values if not pd.isna(x))
    ...
    if len(accessibility_df) > 0:
        ...
        sample_dest = sample_row['Destino_ID']
    ...
    for _, acc_row in accessibility_df.iterrows():
        ...
        destination_zone = acc_row['Destino_ID']  # Changed from 'Zona_Mapeada' to 'Destino_ID'
        ...
        if not od_match.empty:
            ...
            if uses_public_transport:
                ...
                # Direct journey - only walking
                # Use travel time from OD data (assuming it's already calculated as walking time in minutes)
                access_time_minutes = travel_time  # All time is access walking
                egress_time_minutes = 0
                total_time = travel_time
                access_distance = np.nan  # No specific distance lookup available
                egress_distance = 0

                journey_type = "Walking Only"

            # Add accessibility scores for this origin-destination pair (using Mejor_* columns)
            accessibility_scores = {}
            for col in accessibility_df.columns:
                if col.startswith('Mejor_'):  # Changed from 'Mapeada_' to 'Mejor_'
...
def analyze_accessibility_coverage(results_df, destination_type='all'):
    ...
    if 'Journey_Type' in results_df.columns:
        journey_breakdown = results_df['Journey_Type'].value_counts()
        print(f"\nJourney Type Breakdown:")
        for jtype, count in journey_breakdown.items():
            print(f"  {jtype}: {count} ({count / len(results_df) * 100:.1f}%)")

    # Time statistics (only for valid journeys)
    valid_journeys = results_df.dropna(subset=['Access_Time_Minutes'])
    if not valid_journeys.empty:
        ...
        # Only calculate egress statistics for public transport journeys
        pt_journeys = valid_journeys[valid_journeys['Uses_Public_Transport'] == True]
        if not pt_journeys.empty:
            print(f"Average egress time (PT journeys): {pt_journeys['Egress_Time_Minutes'].mean():.2f} minutes")

        if 'Total_Time_Minutes' in valid_journeys.columns:
...
def save_results(results_df, output_file='accessibility_with_times_consultas_generales.csv'):
...
if __name__ == "__main__":
    ...
    accessibility_file = r"C:\Users\avoltan\Desktop\OD_walk_data\consultas_generales_accessibility.csv"
    ...
    if not results.empty:
        ...
        sample_cols = ['Origin', 'Destination', 'Journey_Type', 'Access_Time_Minutes', 'Egress_Time_Minutes',
                       'Total_Time_Minutes']
        ...
        # Save results
        save_results(results, 'accessibility_with_times_consultas_generales.csv')
        ...
        if 'Mejor_JRT' in results.columns:
            ...
            corr_data = results.groupby('Origin').agg({
                ...
                'Mejor_JRT': 'first',  # Accessibility scores are same for all destinations from same origin
                'Mejor_NTR': 'first',
                'Origin_Population': 'first'
            }).dropna()

            if len(corr_data) > 1:
                correlation = corr_data['Access_Time_Minutes'].corr(corr_data['Mejor_JRT'])
