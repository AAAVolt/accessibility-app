...
def load_and_process_data(uploaded_file, stop_mapping=None):
    ...
    try:
        ...
        df = determine_journey_direction(df)
...
def determine_journey_direction(df):
...
def create_timetable_plot(df, selected_journeys=None, time_range=None, direction_filter=None):
    ...
    # Get unique stops and sort them by first appearance
    stops_order = []
...
def main():
    ...
    with st.sidebar:
        ...
        if uploaded_file is not None:
            ...
            with st.spinner("Processing journey data..."):
                df = load_and_process_data(uploaded_file, stop_mapping)
    ...
    if uploaded_file is not None and 'df' in locals() and df is not None:
        ...
        with tab3:
            ...
            - **Y-axis**: Stops (with names if stop list provided, otherwise numbers)
            ...
            - **Hover**: Hover over points to see detailed information including stop names
